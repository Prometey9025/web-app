...
export const PCompetitionsList = () => {
  ...
  useEffect(() => {
    const fn = async () => {
      ...
      const contestsServer = await (await fetch('/api/contests')).json()
    ...
    <Grid container spacing={3}>
      <Grid item xs={10}>
        <Box sx={{
          ...
          <Box component='h2' sx={{ml: '15px'}}>Соревнования</Box>
          <Button 
            component={Link}
            to={'create'}
            variant='contained' 
        ...
        </Box>
        {
          contests.length ? (
            <Grid container spacing={3} columns={matchesXL ? 3 : matchesMD ? 2 : 1}>
              {contests.map(item => {
                const isSuccess = data.infoRating.competitionsEnded.find(element => parseInt(element) === parseInt(item.id))
